...
#define GMTK_SWZ3_OP(op)																				\
	vec<3, T> operator op (const swizzle3<a, b, c, T> &s) const											\
	{																									\
		vec<3, T> res(static_cast<T>(0));																\
	...
	vec<3, T> operator op (const swizzle3<a2, b2, c2, T> &s) const										\
	{																									\
		vec<3, T> res(static_cast<T>(0));																\
		...
		return res;																						\
																										...
																										\
	vec<3, T> operator op (const T &v) const															\
	{																									\
		vec<3, T> res(static_cast<T>(0));																\
		...
		return res;																						\
																										...
																										\
	friend vec<3, T> operator op (const T &l, const swizzle3<a, b, c, T> &r)							\
	{																									\
		vec<3, T> res(static_cast<T>(0));																\
...
#define GMTK_SWZ3_ROP(op)																				\
	writable_type &operator op (const swizzle3<a, b, c, T> &s)											\
	{																									\
		...
		return *this;																					\
																										...
																										\
	template<int a2, int b2, int c2, typename T>														\
	writable_type &operator op (const swizzle3<a2, b2, c2, T> &s)										\
	{																									\
		...
		return *this;																					\
																										...
																										\
	writable_type &operator op (const vec<3, T> &s)														\
	{																									\
...
#define GMTK_SWZ3_BOP(op) 																				\
	...
	static inline vec<3, T> operator op (const vec<3, T> &v, const swizzle3<a, b, c, T> &s)	\
	{ 																									\
		vec<3, T> res; 																					\
...
#define GMTK_SWZ3_BROP(op) 																				\
	template<int a, int b, int c, typename T> 															\
	static inline vec<3, T> &operator op (vec<3, T> &v, const swizzle3<a, b, c, T> &s)					\
	{ 																									\
		v.data[0] op s[a]; 																				\
		v.data[1] op s[b]; 																				\
		v.data[2] op s[c]; 																				\
...
#define GMTK_SWZ3_TYPE \
	template<int a, int b, int c> using swz3 = swizzle3<a, b, c, T>;
...
{////
	...
	public:
		...
		typedef typename std::conditional<is_writable, swizzle3<a, b, c, T>, struct duplicate_components_swizzle>::type writable_type;
	...
	template <int a, int b, int c, typename T>
	inline std::ostream &operator<<(std::ostream& os, const swizzle3<a, b, c, T>& s)
	{
