...
#define GMATH_VEC3_LOOP(oper) GMATH_UNROLL_LOOP(i,3,oper)

#define GMATH_VEC3_OPERATOR(oper) \
		{ vec<T,3> res; \
		GMATH_VEC3_LOOP(res[i] = oper); \
		return res; }

#define GMATH_VEC3_REF_OPERATOR(oper) \
		{ GMATH_VEC3_LOOP(oper); \
		return *this; }
...
{////
	...
	template <typename T> struct vec < T, 3 >
	{
		...
		inline vec(const vec<T, 3>& v) {
			GMATH_VEC3_LOOP(data[i] = v.data[i]);
		}

		// Explicit type-conversion copy constructor
		template<typename U> explicit inline vec(const vec<U, 2>& v) {
			GMATH_VEC3_LOOP(data[i] = static_cast<T>(v[i]));
		}

		// Fill constructor
		explicit inline vec(const T& s) {
			GMATH_VEC3_LOOP(data[i] = s);
		}

		// Array initializer
		explicit inline vec(const T* a) {
			GMATH_VEC3_LOOP(data[i] = a[i]);
		}
		...
		inline vec<T, 3> operator-() const {
			GMATH_VEC3_OPERATOR(-data[i]);
		}

		// Component-wise vector multiplication
		inline vec<T, 3> operator*(const vec<T, 3>& v) const {
			GMATH_VEC3_OPERATOR(data[i] * v.data[i]);
		}
		...
		inline vec<T, 3>& operator*=(const vec<T, 3>& v) {
			GMATH_VEC3_REF_OPERATOR(data[i] *= v.data[i]);
		}
		...
		inline vec<T, 3>& operator*=(const T& s) {
			GMATH_VEC3_REF_OPERATOR(data[i] *= s);
		}
